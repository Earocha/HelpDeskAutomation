    ...
    async Start(request){
        ...
        switch(request.body.type){
            ...
            case 'item.create':
                    ...
                    var Data = await this.getPodioFuntion.GetItemPerIdAmandaApp(request.body.item_id)
                    ...
                    var createItem = await this.getPodioFuntion.CreateItemFastlyApp(OrgData);
                    var NewData = await this.OrganizeDataOnCreate(createItem,createItem.item_id);
                    console.log("Item Created On Amanda App===>>>",NewData)

                    
                    var updateItem = await this.getPodioFuntion.UpdateItemAmandaApp(NewData,request.body.item_id);
            ...
            case 'item.update':
                    ...
                    var Data = await this.getPodioFuntion.UpdateItemFastlyApp(OrgData, request.body.item_id)
                    var OrgData = await this.OrganizeData(Data);

                    if(OrgData.length !== 0 ){
                        console.log("Item Updated on fastly!")
                    }
            ...
            case 'file.change':
                    ...
                    for(var i in OrgData,OrgData[0].FileIds){
                        ...
                        var new_file_id = await this.getPodioFuntion.CopyFileFastly(OrgData[0].FileIds[i])
                        file_id.push(new_file_id.file_id);
                    }
                    //console.log("new files =>", file_id);
                    //update other apps function

                    var Data = await this.getPodioFuntion.FileUploadOnFastly(OrgData, request.body.item_id , file_id)
                    var OrgData = await this.OrganizeData(Data);
